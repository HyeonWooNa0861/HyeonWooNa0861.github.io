...
- 원본 자료는 원래의 로컬 작업 위치에 보존하고, 게시 대상 원문 PDF는 collection별 `assets/pdfs/` 경로에도 복제해 포스트에서 연결한다. 사용자가 직접 제공한 발표·분석 슬라이드의 게시를 명시적으로 요청한 경우에는 해당 제공본을 원문 첨부 대상으로 처리하되, 개인정보와 출처 metadata를 먼저 확인한다. 외부에서 별도로 내려받은 제3자 자료는 인터넷에 공개되어 있다는 사실만으로 재배포 허가를 추정하지 않으며, 라이선스·직접 허가·공개 도메인 또는 공식 저장소의 배포 조건이 확인되지 않으면 파일을 복제하지 않고 공식 원문 URL을 연결한다.
- `research_files`와 transfer 작업에서 생성한 전체 번역본 파일은 로컬 산출물로 보존하고 블로그 자산으로 업로드하지 않는다. 공개 글에는 원문을 토대로 새로 구성한 한국어 해설·분석을 싣는다. 전체 번역 공개는 사용자가 별도로 요청하고 번역·재배포 근거까지 확인한 예외에만 허용한다.
...
- 수식 기호와 변수명은 원본 PDF notation을 우선하되, 오기·모순을 그대로 복제하지 않는다. 아래 `원자료 정확성 검증과 정정` 기준에 따라 표기 관례와 실제 오류를 먼저 구분한다.
...
9. 긴 유도식은 본문 속 inline 수식에 넣지 말고 display block으로 분리한다. 좁은 화면에서 긴 display 식은 수식 영역 안에서 가로 스크롤되게 하며 페이지 전체를 늘리거나 축소시키지 않는다. Sidebar가 있는 Study 페이지도 모바일에서 본문이 한 열 전체 폭을 쓰는지, 복습 답변을 펼친 상태까지 확인한다.
...
- 외부에서 별도로 수집한 이미지, 도표, 코드, PDF, 번역문은 소유권·라이선스·직접 허가·공개 도메인 등 재배포 근거가 확인된 범위에서만 복제한다. 공개 웹에서 열람할 수 있다는 사실은 그 자체로 재배포 허가가 아니다. 단순 열람만 허용된 원문은 공식 원문 링크와 짧은 인용을 사용한다.
- 사용자가 직접 제공한 발표·분석 자료의 파일 게시를 명시적으로 요청한 경우에는 그 제공본만 게시 범위로 본다. 자료가 인용하는 외부 논문·도표·부속 파일까지 같은 권한이 있다고 확장하지 않고 각각 확인한다.
- 인용에는 출처와 원문 링크를 남기고, 수정·번역·재구성했다면 그 사실을 밝힌다. 출처, 제공 경위, 파일 hash와 확인한 라이선스는 로컬 sidecar·README 또는 publish log에 보존한다. 공개 본문에는 실제 원문 링크와 필요한 저자·라이선스 표기만 간결하게 두며, 라이선스가 공개 고지를 요구하지 않는 한 권리 검토 과정이나 `not redistributed` 같은 운영 문구를 반복하지 않는다. 공개 전체 번역 예외에는 게시 권한의 근거를 `번역·게시 권한` 항목에 기록한다.
...
- 게시 전 `Source PDF` 또는 `Source Images` 표기와 하단 자료 섹션이 서로 일치하는지 확인한다. 첨부한 원문은 실제 내부 자산으로 연결하고, 외부 수집 자료 중 재배포 근거가 확인되지 않은 파일은 공식 원문 URL만 연결한다. 공개 글에는 `locally supplied; not redistributed` 같은 획일적인 비공개 운영 문구를 넣지 않고, 독자가 알아야 할 구체적인 출처·접근 제한이 있을 때만 짧게 설명한다.
- 특히 Study 자료는 원문 대비 누락이 생기기 쉬우므로, `전체 흐름` 표가 원문 전체 범위를 대표하는지 마지막에 다시 점검한다.

### 원자료 정확성 검증과 정정

- 강의 슬라이드, 교재, 논문과 공식 문서도 사람이 작성한 자료이므로 사실성을 자동으로 보장하지 않는다. **원문 충실성 검사와 내용 정확성 검사는 별개**이며, 원문과 같다는 이유만으로 올바르다고 판정하지 않는다.
- 핵심 주장·수식·수치·그림을 원문 page/slide 또는 timestamp에 연결하고, 정의와 가정, 기호·단위, 부호·첨자·상수, 예제의 재계산, 경계값·반례와 본문 간 일관성을 검토한다. 추출 텍스트에서만 발견된 이상은 원본 화면과 먼저 대조해 OCR·추출 오류인지 확인한다.
- 불일치는 `원문 오류`, `표기 관례 차이`, `가정이 생략된 단순화`, `전사·번역 오류`, `미확인`으로 구분한다. Fourier 부호·정규화처럼 동치인 관례는 서로 변환한 뒤 비교하며, 차이가 있다는 이유만으로 원문 오류라고 단정하지 않는다.
- 수학적 주장은 독립적인 유도·대입·반례로 검증한다. 물리·실험·역사·구현에 관한 주장은 해당 분야의 공식 정정문, 표준, 교재, 원 논문 등 적절한 1차 근거와 교차 확인한다. 동일 주장을 재인용한 여러 페이지는 독립된 근거로 세지 않으며, 근거의 적용 조건도 함께 확인한다.
- 확인된 오류는 블로그 본문에 올바른 내용을 쓰고, 학습에 영향을 주면 인접한 짧은 정정 설명 또는 `Source Check` 표에 원문 위치, 문제, 수정, 근거를 남긴다. 원본 파일은 수정하지 않으며 원저자의 공식 정정과 작성자가 확인한 오류를 구분한다.
- 판정이 끝나지 않은 주장은 사실처럼 단정하지 않는다. 핵심 결론에 영향을 주는 미확인 오류가 남으면 해당 자료의 새 commit·push·배포를 보류하고 검증 범위와 미확인 항목을 보고한다. 단순한 적용 범위 제한은 조건을 명시하되 오류 정정과 혼동하지 않는다.
- 검토 결과에는 확인한 원자료·포스트, 페이지·주제 범위, 발견·수정한 항목과 남은 공백을 기록한다. 자동 validator·수식 렌더링 통과는 문법·표시 증거일 뿐 사실 정확성의 증거가 아니며, 일부만 검토한 경우 전체 자료에 오류가 없다고 보고하지 않는다.
...
- 새 보강 설명에도 같은 기준을 적용한다. 식을 소개하기 전에 **왜 이 개념·표현·변환이 필요한지, 어떤 문제를 푸는지** 설명하고, `목적 → 정의·가정 → 중간 유도 → 결론 → 예제·검산 → 해석·적용 범위`가 이어지게 작성한다. 기존 보강을 수정할 때도 관련 구간의 생략된 논리를 함께 점검한다.
...
12. 원문 자체의 오류 가능성을 독립적으로 검토하고, 확인된 오류·관례 차이·미확인 주장을 구분했는가?
13. 원문 설명과 새 보강 모두에서 개념·표현을 도입하는 이유와 유도 후 예제 또는 검산을 제공했는가?
...
- 원문이 PDF이면 상단에 `Source PDF:` 형식으로 파일명을 명시하고, 사용자가 제공해 게시를 요청한 원문은 하단 `## PDF`에서 같은 공개 자산을 링크한다. 외부에서 별도로 수집해 재배포 근거가 확인되지 않은 PDF는 파일명과 공식 원문 URL을 `## Source Materials`에 연결하며, 획일적인 `locally supplied; not redistributed` 문구는 넣지 않는다.
...
- 원문 PDF가 여러 개라면 상단에는 `Source PDFs:` 아래 목록을 두고, 첨부 대상 원문은 하단 `## PDF`의 내부 자산에 각각 연결한다. 외부 수집 파일 중 재배포 근거가 확인되지 않은 항목은 같은 목록에서 공식 원문 URL로 연결한다. 다른 유형의 원문이 여러 개라면 같은 원칙을 `Source Materials`와 대응하는 하단 섹션에 적용한다.
...
- 원문 논문 전체를 축어적으로 번역한 로컬 파일은 게시하지 않는다. 본문에는 논문 전체 구조를 따라가되 문장과 설명을 새로 구성한 `한국어 번역형 해설`을 제공한다.
- `한국어 번역형 해설`은 초록/서론, 방법, 실험, 결론/한계를 빠짐없이 따라가되 문장과 표현은 새로 구성한다.
- 원문 고유명사, 모델명, 수식 기호, 데이터셋, 실험 수치, DOI, source PDF 링크는 유지한다.
- 사용자가 전체 번역 공개를 별도로 요청하고 그 권한이 명시적으로 확인된 경우에만 축어 번역을 게시할 수 있으며, 그 경우에도 허가 범위나 라이선스 정보를 참고자료에 남긴다.
- 게시 대상 Research 원문 PDF는 `assets/pdfs/research/<slug>/<slug>.pdf` 아래에 두고 포스트 하단 `참고자료`에서 같은 경로로 연결한다. 외부에서 수집한 원문에 재배포 근거가 없으면 파일 대신 공식 원문 URL을 연결한다.
...
전체 번역본 Markdown/PDF는 위 공개 자산 경로에 두지 않는다. 원문과 번역본이 함께 로컬에 있어도 블로그에는 게시 대상 원문과 새로 작성한 해설 포스트만 포함한다.
...
- `원자료 정확성 검증과 정정`에 따라 원문의 사실성도 검토했는가? 검토 범위·수정 근거·미확인 항목이 기록되어 있으며 핵심 결론에 영향을 주는 미확인 오류가 남아 있지 않은가?
- Study 글은 원문 수식 목록과 본문 대응 구간을 대조하고, `Study 수식 검증 체크리스트`의 가정·유도·표기·단위·직관·실패 조건·출처 항목을 모두 확인했는가?
- 첨부한 원문·인용·이미지의 게시 범위와 외부 수집 자료의 재배포 근거를 구분해 확인했는가? 전체 번역본 파일이 공개 자산에 포함되지 않았는가?
