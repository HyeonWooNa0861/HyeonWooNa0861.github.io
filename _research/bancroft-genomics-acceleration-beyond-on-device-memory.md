...
Reference comparison에서 `offset mod 4`에 따라 미리 shift해 둔 reference copy를 고르면, 비트 shift를 반복하지 않고 byte-aligned `memcmp`를 사용할 수 있다. 논문은 이 최적화가 encoding performance를 평균 4배 높인다고 보고한다.
