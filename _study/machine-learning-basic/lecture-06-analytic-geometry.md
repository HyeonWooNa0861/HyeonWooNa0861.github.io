...
inner product에서 norm, distance, angle을 차례로 계산해 각 정의가 앞 개념에 의존하는 흐름을 잡는다. SPD 조건 xᵀAx>0과 orthogonal 조건 AᵀA=I를 섞지 말고, 후자가 길이와 각도를 보존함을 직접 계산한다. projection은 column space에서 가장 가까운 점을 찾는 절차로 재현해 least squares와의 연결까지 확인한다.
