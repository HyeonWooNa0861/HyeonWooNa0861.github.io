...
| PDF 페이지 | 중요 최적화식 | 본문 대응 |
|---:|---|---|
| 3–7 | $$\min_x f(x)$$, GD update, 수렴성과 step size | 1–5, 17.1 |
| 8–9 | momentum update | 6 |
| 10–12 | SGD·mini-batch gradient 근사 | 7, 8 |
| 13–14 | 2차 Taylor 근사와 Newton update | 9, 10, 17.2 |
| 15–20 | convex set/function 정의, $$x^2$$·negative entropy 예시, 닫힘 성질 | 11–16, 17.3 |

페이지 21은 convex 함수의 성질을 마무리하며 별도 Q&A 페이지는 없다. SGD·mini-batch는 full gradient의 **근사**이고, Newton 식은 local Taylor quadratic model을 풀어 얻는 update로 17절에서 구분한다.
...
### 17.1 Gradient descent가 내려가는 조건

$$f$$의 gradient가 $$L$$-Lipschitz라고 가정하면 descent lemma로

$$
f(x+s)\le f(x)+\nabla f(x)^Ts+\frac{L}{2}\lVert s\rVert_2^2
$$
...
$$
f(x-\alpha\nabla f(x))
\le f(x)-\alpha\left(1-\frac{L\alpha}{2}\right)\lVert\nabla f(x)\rVert_2^2.
$$

따라서 $$0<\alpha<2/L$$이면 stationary point가 아닌 곳에서 상계가 감소한다. 이는 해당 smoothness 가정 아래의 **정확한 보장**이지, 모든 함수와 모든 step size에서 loss가 감소한다는 경험 법칙이 아니다.

### 17.2 Newton update 재유도

2회 미분 가능하고 현재 Hessian $$H_k$$가 가역이라고 하자. 2차 Taylor **근사**

$$
q(s)=f(\theta_k)+g_k^Ts+\frac12s^TH_ks
$$

의 gradient는 $$g_k+H_ks$$다. 이를 0으로 두면 $$s=-H_k^{-1}g_k$$, 즉 $$\theta_{k+1}=\theta_k-H_k^{-1}g_k$$다. $$H_k$$가 positive definite일 때만 이 stationary point가 이차 근사의 최소점이며, indefinite Hessian이면 하강 방향을 보장하지 않는다.

### 17.3 Convex local minimum은 global minimum

$$x^*$$가 local minimum인데 더 낮은 점 $$y$$가 있다고 가정하자. 충분히 작은 $$t\in(0,1)$$에 대해 $$z_t=(1-t)x^*+ty$$는 $$x^*$$ 근방에 있다. Convexity로

$$
f(z_t)\le(1-t)f(x^*)+tf(y)<f(x^*)
$$

이므로 local minimum 정의와 모순이다. 따라서 모든 local minimum은 global minimum이다. 정의역이 convex이고 함수가 convex라는 조건이 모두 필요하다.

$$\alpha$$는 parameter 단위/gradient 단위를 가져 update의 차원을 맞춘다. loss가 무차원이고 parameter가 $$u$$ 단위라면 gradient는 $$u^{-1}$$, Hessian은 $$u^{-2}$$ 단위다. 딥러닝의 learning rate는 구현상 숫자로 취급되지만 feature scaling에 민감한 이유가 여기에 있다.
