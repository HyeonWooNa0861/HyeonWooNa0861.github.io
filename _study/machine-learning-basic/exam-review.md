...
- `machine-learning-basic-final-assignment.pdf`
...
기말과제는 시험 범위의 계산형 포인트를 거의 그대로 압축한다. Bayes 정리, 이항분포와 기대값, gradient descent update, convex 판정, polynomial regression의 MLE와 예측분포를 한 번에 확인할 수 있다.

### 6.1 Bayes 정리와 이항분포

불량 여부를 \\(D\\), 검사 결과를 \\(T\\)라고 하자. 불량률은 \\(P(D)=0.08\\)이고, 검사는 다음을 만족한다.
...
<details>
<summary>1. 양성 판정을 받은 부품이 실제 불량일 확률 \\(P(D\mid T=+)\\)를 구하라.</summary>
...
답변: 양성 판정을 받은 부품이 실제 불량일 확률은 \\(\frac{85}{223}\approx 0.3812\\)이다.

</details>

<details>
<summary>2. 양성 판정을 받은 서로 독립인 30개 부품 중 실제 불량품 수의 기대값을 구하라.</summary>

풀이과정:

양성 판정을 받았다는 조건 아래에서 각 부품이 실제 불량일 확률은 앞에서 구한 값이다.
...
답변: 기대 불량품 수는 \\(\frac{2550}{223}\approx 11.435\\)개다.

</details>

<details>
<summary>3. 양성 판정을 받은 서로 독립인 3개 부품 중 정확히 2개가 실제 불량일 확률을 구하라.</summary>
...
Step size는 \\(\eta=\frac{1}{5}\\)이고, update는 다음과 같다.
...
여기서 \\(\nabla_\theta f(\theta)\\)는 row vector로 둔다.

<details>
<summary>4. \\(\theta^{(0)}=\begin{bmatrix}0\\0\end{bmatrix}\\)일 때 한 번 update한 결과를 구하라.</summary>
...
<details>
<summary>5. \\(\theta^{(0)}=\begin{bmatrix}2\\-1\end{bmatrix}\\)일 때 한 번 update한 결과를 구하라.</summary>

풀이과정:

\\(\theta^{(0)}=(2,-1)^T\\)에서 gradient는
...
<details>
<summary>6. \\(f(x)=\lvert x\rvert\\), \\(x\in\mathbb{R}\\)가 convex인지 판단하라.</summary>

풀이과정:

임의의 \\(x,y\in\mathbb{R}\\), \\(0\le\lambda\le 1\\)에 대해 삼각부등식을 사용한다.
...
<details>
<summary>7. \\(g(x)=\sin x\\), \\(x\in\mathbb{R}\\)가 convex인지 판단하라.</summary>

풀이과정:

Convex가 아님을 보이려면 정의를 깨는 반례 하나를 제시하면 된다. \\(x=0\\), \\(y=\pi\\), \\(\lambda=\frac{1}{2}\\)를 잡자.
...
2차 다항식 회귀를 위해 feature vector를 다음처럼 정의한다.
...
<details>
<summary>8. MLE \\(\theta_{\mathrm{ML}}\\)을 구하라.</summary>

풀이과정:

Feature matrix와 target vector는 다음과 같다.
...
Gaussian noise의 variance \\(0.2\\)는 positive constant이므로 MLE는 squared error를 최소화하는 해와 같다.
...
이 데이터는 3개의 점을 2차 다항식으로 정확히 지날 수 있다. 식으로 쓰면
...
두 번째 식에서 \\(\theta_0=1\\)이다. 첫 번째와 세 번째 식을 더하면 \\(2\theta_0+2\theta_2=4\\)이므로 \\(\theta_2=1\\)이다. 세 번째 식에서 \\(\theta_1=0\\)이다.
...
<details>
<summary>9. 새로운 입력 \\(x_*=2\\)에 대한 출력 \\(y_*\\)의 예측분포를 구하라.</summary>

풀이과정:

먼저 feature vector를 계산한다.
...
모델에서 observation variance가 \\(0.2\\)로 주어졌으므로
...
8. 기말과제 기반 실전 문제는 실제 계산 연습용으로 다시 풀어본다. 특히 Bayes 정리의 분모 계산, GD update의 부호, convex 반례, \\(\Phi\\) 행렬 구성은 실수하기 쉬운 지점이다.
