...
| 원문 PDF | 확인 페이지 | 주요 수식·주제 | 이 글의 대응 위치 |
|---|---:|---|---|
| Lecture 11 | 4–23 | 확률 공리, PMF, joint/marginal/conditional, CDF/PDF | 1.1–1.6 |
| Lecture 12 | 11–27 | 독립성, Bayes 정리, 기대값·분산·공분산 | 1.7–1.10 |
| Lecture 13 | 3–20 | Gaussian, 조건부·주변 분포, 선형변환, Jacobian 보정 | 1.11–1.14 |
| Lecture 14 | 3–21 | GD/SGD, momentum, Newton update, convexity | 2.1–2.6 |
| Lecture 15 | 6–22 | 결정론적·확률적 모델, ERM, MLE, MAP, CV | 3.1–3.6 |
| Lecture 16 | 2–18 | ERM/MAP 복습, 확률적 모델링, DGM, model selection | 3.1–3.7 |
| Lecture 17 | 2–17 | Gaussian 회귀 모델, MLE/MAP 정규방정식, RMSE | 4.1–4.4 |

페이지는 각 PDF의 물리 페이지 기준이다. 시험 정리에서 새로 제시한 증명·조건부 Gaussian 유도는 원문 슬라이드의 표기를 복제한 것이 아니라, 해당 결론이 성립하는 조건을 보충한 해설이다.
...
다음 표는 이 글의 확률·최적화·회귀식을 한 번에 검산하기 위한 **보충 해설**이다. 표본 수를 $$N$$, feature 수를 $$d$$, 입력 feature $$j$$의 단위를 $$U_{x_j}$$, 출력의 단위를 $$U_y$$, scalar loss의 단위를 $$U_L$$이라 하자. 별도의 scalar 확률변수 $$Z,W$$의 단위는 각각 $$U_Z,U_W$$로 쓴다. 표준화된 순수 수치 데이터라면 해당 단위는 모두 무차원으로 볼 수 있다.

| 양 | 크기 | 단위 | 호환성 확인 |
|---|---:|---|---|
| $$x_n$$ | $$d\times1$$ | 성분별 $$U_{x_j}$$ | 서로 다른 feature 단위를 한 벡터에 담을 수 있지만 내적의 각 항은 같은 출력 단위가 되어야 한다. |
| $$X$$ | $$N\times d$$ | 열 $$j$$마다 $$U_{x_j}$$ | 행은 sample, 열은 feature다. |
| $$y,\hat y,\epsilon$$ | $$N\times1$$ | $$U_y$$ | $$y= X\theta+\epsilon$$에서 세 항의 shape와 단위가 같아야 한다. |
| $$\theta$$ | $$d\times1$$ | 성분 $$j$$마다 $$U_y/U_{x_j}$$ | $$x_{nj}\theta_j$$의 모든 항이 $$U_y$$여야 합할 수 있다. |
| bias $$\theta_0$$ | scalar | $$U_y$$ | $$x_n^T\theta$$와 더하므로 출력과 같은 단위다. |
| $$\operatorname{Cov}(Z,W)$$ | scalar | $$U_ZU_W$$ | 중심화한 두 변수의 곱 평균이므로 두 단위가 곱해진다. |
| $$\Sigma=\operatorname{Cov}(x)$$ | $$d\times d$$ | 원소 $$(i,j)$$는 $$U_{x_i}U_{x_j}$$ | 대각은 $$U_{x_i}^2$$인 variance, 비대각은 pair 단위의 covariance다. |
| $$\sigma^2$$ | scalar | $$U_y^2$$ | Gaussian regression noise variance이며 squared residual과 호환된다. |
| $$\sigma$$ | scalar | $$U_y$$ | noise standard deviation이며 residual·RMSE와 직접 비교할 수 있다. |
| MSE | scalar | $$U_y^2$$ | $$N^{-1}\sum_n(y_n-\hat y_n)^2$$는 squared output 단위다. |
| RMSE | scalar | $$U_y$$ | $$\sqrt{\mathrm{MSE}}$$라서 target과 같은 단위로 돌아온다. |
| $$L(\theta)$$ | scalar | $$U_L$$ | loss 정의에 따라 무차원일 수도, squared output 단위일 수도 있다. |
| $$\nabla_\theta L$$ | $$d\times1$$ | 성분 $$j$$는 $$U_L/U_{\theta_j}$$ | parameter와 같은 shape여야 한다. |
| learning rate $$\eta$$ | scalar 또는 좌표별 | 좌표 $$j$$에서 $$U_{\theta_j}^{2}/U_L$$ | $$\eta(\nabla_\theta L)_j$$가 $$U_{\theta_j}$$가 되어 update와 더해져야 한다. 단일 scalar는 parameter 단위가 서로 호환될 때 자연스럽다. |

핵심 규칙은 **더하거나 빼는 양은 shape뿐 아니라 단위도 같아야 한다**는 것이다. 예를 들어 $$x_n^T\theta$$가 scalar라는 차원 검사만 통과해도 충분하지 않다. 각 곱 $$x_{nj}\theta_j$$가 모두 $$U_y$$여야 합이 물리적으로 의미가 있고, 그래야 $$y_n-x_n^T\theta$$, RMSE, Gaussian noise의 $$\sigma$$도 같은 출력 scale에서 비교할 수 있다. 또한 learning rate는 관습적으로 무차원 scalar로 구현되지만, 단위가 있는 모델을 엄밀히 해석하면 gradient를 parameter 변화량으로 바꾸는 scale이므로 위 표의 호환 단위를 가져야 한다. feature/target을 표준화하면 이 단위 문제가 무차원 좌표계로 숨겨질 뿐, shape 호환성은 그대로 남는다.
...
이 inverse 형태는 $$\Sigma_{yy}$$가 가역, 즉 관측 블록이 비퇴화 Gaussian일 때의 식이다. $$\Sigma_{yy}$$가 singular이면 이 표준 inverse 공식을 그대로 쓸 수 없고, 분포가 놓인 부분공간에서 generalized conditional Gaussian 또는 Moore--Penrose pseudoinverse를 사용하는 별도 처리가 필요하다.
...
#### 완전제곱으로 조건부 Gaussian을 얻는 증명 개요

> **작성자 보충:** 아래는 공식을 암기하는 데서 그치지 않고 왜 Schur complement가 나타나는지 보여 주는 계산 개요다.

Joint Gaussian density의 지수는 precision matrix $$\Lambda=\Sigma^{-1}$$를 블록으로 나누면, 상수항을 제외하고
...
이다. $$y$$를 관측값으로 고정하고 $$x$$에 의존하는 항만 모아 $$\delta_x=x-\mu_x$$, $$\delta_y=y-\mu_y$$라 쓰면

$$
-\frac12\left(
\delta_x^T\Lambda_{xx}\delta_x
+2\delta_x^T\Lambda_{xy}\delta_y
\right)+\mathrm{const}(y).
$$

$$\Lambda_{xx}$$가 가역일 때 완전제곱하면

$$
-\frac12
\left(\delta_x+\Lambda_{xx}^{-1}\Lambda_{xy}\delta_y\right)^T
\Lambda_{xx}
\left(\delta_x+\Lambda_{xx}^{-1}\Lambda_{xy}\delta_y\right)
+\mathrm{const}(y).
$$

따라서 조건부분포의 평균과 공분산은 우선

$$
\mu_{x\mid y}=\mu_x-\Lambda_{xx}^{-1}\Lambda_{xy}(y-\mu_y),
\qquad
\Sigma_{x\mid y}=\Lambda_{xx}^{-1}
$$

로 읽힌다. Block inverse 항등식을 적용하면

$$
-\Lambda_{xx}^{-1}\Lambda_{xy}=\Sigma_{xy}\Sigma_{yy}^{-1},
\qquad
\Lambda_{xx}^{-1}=\Sigma_{xx}-\Sigma_{xy}\Sigma_{yy}^{-1}\Sigma_{yx}
$$

이므로 위의 익숙한 조건부 Gaussian 공식과 정확히 일치한다. 이 유도는 joint covariance가 positive definite이고 $$\Sigma_{yy}$$가 가역인 비퇴화 Gaussian을 가정한다. Singular covariance에서는 Lebesgue density와 통상적인 inverse 기반 완전제곱을 그대로 쓸 수 없다. 각 평균·관측값은 해당 변수와 같은 단위이고, $$\Sigma_{xy}\Sigma_{yy}^{-1}$$는 `x 단위/y 단위`이므로 평균 보정항의 단위도 $$x$$와 같다.
...
이 update는 2차 근사의 최소점을 실제로 풀어 얻는다. $$\Delta=\theta-\theta_t$$라 두면 Taylor quadratic model은

$$
q_t(\Delta)
=f(\theta_t)+g_t^T\Delta+\frac12\Delta^TH_t\Delta
$$

이다. $$H_t$$가 symmetric positive definite이면 $$q_t$$는 strictly convex이고, $$\Delta$$에 대한 gradient는

$$
\nabla_{\Delta}q_t(\Delta)=g_t+H_t\Delta.
$$
...
\Delta^*=-H_t^{-1}g_t,
\qquad
\theta_{t+1}=\theta_t+\Delta^*=\theta_t-H_t^{-1}g_t
$$

를 얻는다. 실제 구현에서는 inverse를 만들기보다 linear system $$H_t\Delta=-g_t$$를 푸는 편이 안정적이다. $$H_t$$가 singular하면 유일한 step이 없고, indefinite이면 stationary point가 quadratic model의 최소점이 아닐 수 있으므로 damping, trust region, modified Hessian이 필요하다. Taylor 식은 일반 비이차 함수에서는 local **근사**지만, 목적함수가 정확한 quadratic이고 Hessian이 SPD이면 이 전개가 전역적으로 정확해 한 step에 minimizer에 도달한다. $$g_t$$의 좌표 단위가 $$U_f/U_{\theta_j}$$, Hessian 원소 단위가 $$U_f/(U_{\theta_i}U_{\theta_j})$$이면 $$H_t^{-1}g_t$$는 parameter와 같은 단위를 가져 update가 성립한다.
...
확률밀도의 지수는 무차원이어야 하므로 $$\tau$$는 $$\theta$$와 같은 단위를 가져야 한다. 위 isotropic 식은 모든 $$\theta_j$$가 같은 단위를 갖거나 먼저 무차원화된 좌표에서 자연스럽다. 서로 다른 단위의 parameter를 그대로 다룬다면 $$\sum_j\theta_j^2$$를 단순히 더하는 L2 penalty는 단위가 호환되지 않는다. 이때는 좌표별 scale $$\tau_j$$를 둔 $$\sum_j\theta_j^2/\tau_j^2$$처럼 무차원화한 prior/penalty를 사용해야 한다. 또한 NLL이 무차원일 때 regularization 계수도 penalty 전체가 NLL과 같은 단위를 갖도록 정해야 한다.
