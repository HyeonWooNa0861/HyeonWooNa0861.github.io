...
| 성질 | 의미 |
|---|---|
| `A`가 정사각행렬일 때 invertible iff `rank(A)=n` | full rank면 역행렬 존재 |
| `Ax=b` 해 존재 iff `rank(A)=rank(A|b)` | augmented matrix rank로 consistency 확인 |
| `Ax=0` 해 공간 차원은 `n-rank(A)` | null space 차원 |
...
| 차원이란? | 기저 벡터의 개수 |
| rank-nullity 핵심은? | null space 차원은 `n-rank(A)` |
...
3. `rank(A) < n`이면 `Ax=0`은 어떤 해를 가지는가?
