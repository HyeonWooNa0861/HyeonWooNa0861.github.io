...
```text
raw data -> feature extraction -> x in R^D
```
...
이 관점에서는 기존 데이터셋을 잘 설명하는 함수 `f_theta`를 찾는 것이 학습이다.
...
이 경우 모델은 `y_hat` 하나가 아니라 `y`가 나올 확률분포를 제공한다. 데이터 관측 error, noise, 데이터 부족으로 인한 불확실성을 함께 다룰 수 있다.
...
```text
L(f_theta(x_i), y_i) = (y_i - f_theta(x_i))^2
```

```text
theta_hat = argmin_theta sum_i (y_i - f_theta(x_i))^2
```
...
```text
좋은 training error != 좋은 test error
```
...
```text
objective = empirical risk + lambda * penalty(theta)
```
...
```text
p(D | theta) = product_i p(y_i | x_i, theta)
```

곱은 계산하기 불편하므로 log를 취한다.

```text
log p(D | theta) = sum_i log p(y_i | x_i, theta)
```

MLE는 log-likelihood를 최대화하는 것이고, 실제 최적화에서는 negative log-likelihood를 최소화한다.

```text
theta_hat = argmin_theta - sum_i log p(y_i | x_i, theta)
```
...
```text
objective = negative log-likelihood - log prior
```
