...
$$
P(\mathrm{남성},\mathrm{미착용})=0.60\times 0.70=0.42
$$

$$
P(\mathrm{여성},\mathrm{착용})=0.40\times 0.30=0.12
$$

$$
P(\mathrm{여성},\mathrm{미착용})=0.40\times 0.70=0.28
$$
...
$$
P(X_1=x_1,\ldots,X_n=x_n)=\prod_{i=1}^{n} p_X(x_i)
$$
...
머신러닝에서 training data가 IID sample이라고 말할 때는 보통 모든 데이터가 같은 데이터 생성 과정에서 나왔고, 한 데이터가 다른 데이터의 값을 직접 결정하지 않는다고 가정한다. 이 가정 덕분에 likelihood를 곱 형태로 쓸 수 있다.
...
핵심은 원인에서 결과로 가는 확률을 알고 있을 때, 관측된 결과로부터 원인에 대한 확률을 갱신할 수 있다는 점이다.
...
$$
P(\mathrm{Cancer}\mid\mathrm{Symptom})
=\frac{P(\mathrm{Symptom}\mid\mathrm{Cancer})P(\mathrm{Cancer})}{P(\mathrm{Symptom})}
$$

표를 읽으면 다음 값들을 얻는다.

$$
P(\mathrm{Symptom}\mid\mathrm{Cancer})=1
$$

$$
P(\mathrm{Cancer})=\frac{1}{100000}
$$

$$
P(\mathrm{Symptom}\mid\neg\mathrm{Cancer})=\frac{10}{99999}
$$

$$
P(\neg\mathrm{Cancer})=\frac{99999}{100000}
$$
...
$$
P(\mathrm{Cancer}\mid\mathrm{Symptom})
=\frac{1\times 0.00001}{1\times 0.00001+\frac{10}{99999}\times 0.99999}
=\frac{1}{11}
$$
...
$$
x_1,\ldots,x_n \overset{\mathrm{i.i.d.}}{\sim} P(\cdot\mid\theta)
$$
...
$$
P(F(x_*)\mid X)
=\int P(F(x_*,\theta)\mid\theta)P(\theta\mid X)d\theta
$$
...
$$
\mathbb{E}[X]
=\sum_{i=1}^{6}\frac{1}{6}i
=\frac{1}{6}\sum_{i=1}^{6}i
=\frac{1}{6}\cdot\frac{7\cdot 6}{2}
=3.5
$$
...
$$
\mathbb{E}[X]
=\frac{1}{2}\cdot 6+\sum_{i=1}^{5}\frac{1}{10}i
=3+\frac{1}{10}\cdot\frac{5\cdot 6}{2}
=4.5
$$
...
$$
\operatorname{Cov}(X,Y)
=\int\int (x-\mathbb{E}[X])(y-\mathbb{E}[Y])f_{X,Y}(x,y)dxdy
$$
...
$$
\mathbb{E}[X]=[\mathbb{E}[X_1],\ldots,\mathbb{E}[X_d]]^T
$$
...
\operatorname{Cov}(X)=
\begin{bmatrix}
\operatorname{Var}(X_1) & \cdots & \operatorname{Cov}(X_1,X_d) \\
\vdots & \ddots & \vdots \\
\operatorname{Cov}(X_d,X_1) & \cdots & \operatorname{Var}(X_d)
\end{bmatrix}
$$
...
joint에서 marginal과 conditional을 계산한 뒤 Bayes theorem의 numerator와 evidence denominator가 어떤 역할을 하는지 손으로 전개한다. independence는 joint factorization이고 zero correlation은 선형 관계가 없다는 뜻에 가까우므로 둘을 동치로 두지 않는다. mean·variance·covariance matrix를 계산하고 total variance를 내부 변동과 조건부 평균 변동으로 분해하는 순서까지 복습한다.
