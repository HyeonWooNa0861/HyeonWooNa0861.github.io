...
SVD(Singular Value Decomposition)는 `m x n` 행렬에도 적용할 수 있는 강력한 행렬 분해다.
...
SVD는 `A^T A`와 `A A^T`의 고유값/고유벡터와 연결된다.

| 대상 | 연결 |
|---|---|
| `A^T A` | right singular vectors |
| `A A^T` | left singular vectors |
| 고유값 | singular value의 제곱과 연결 |

`A^T A`와 `A A^T`는 같은 non-zero eigenvalue를 공유한다.
...
| SVD가 고유분해보다 일반적인 이유는? | `m x n` 행렬에도 적용 가능 |
