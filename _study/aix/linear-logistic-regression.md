...
| 2 | 선형 모델 | `score = w^T x + b`는 무엇을 표현하는가? |
...
Logistic regression의 decision boundary는 여전히 `w^T x + b = 0`인 선형 경계다. 달라지는 점은 score를 확률로 해석하고, classification에 맞는 손실 함수를 사용한다는 것이다.
