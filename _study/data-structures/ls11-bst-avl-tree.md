...
> **핵심:** **BST 삭제에서 자식 둘인 노드는 어떻게 처리하는가** 오른쪽 서브트리의 최소 노드로 대체. **BST 최악 시간복잡도가 $$O(n)$$인 이유는** 트리가 한쪽으로 편향되면 높이가 `n`이 됨.
...
BST 삭제는 leaf, 자식 하나, 자식 둘의 세 경우를 먼저 연습하고, 자식 둘이면 오른쪽 서브트리의 최소 노드로 대체하는 과정을 재현한다. 그다음 삽입 경로를 따라 LL·RR·LR·RL을 판정해 단일 회전과 이중 회전 순서를 그림으로 확인한다. BST ordering과 AVL balance condition은 별도 불변식이므로 삭제 대체가 끝난 뒤에도 높이와 balance factor를 다시 계산한다.
...
## AVL 높이가 $$O(\log n)$$인 증명 개요

> **분류:** AVL 균형 조건에서 따르는 **정리**다. 빈 트리 높이를 $$-1$$, 리프 높이를 0으로 두고, $$N(h)$$를 높이 $$h$$인 AVL 트리의 최소 노드 수라 하자.

> **원문 추적:** `LS11_BST_AVL_tree.pdf` p.19는 BST 비용이 높이에 좌우됨을, pp.21–48은 AVL 균형 조건과 네 회전 유형을 제시한다. 최소 노드 점화식과 $$O(\log n)$$ 증명은 원문 결론을 엄밀화한 작성자 보충이다.

노드 수를 최소화하려면 두 서브트리 높이는 균형 조건이 허용하는 가장 큰 차이인 $$h-1$$, $$h-2$$여야 한다. 따라서

$$
N(h)=1+N(h-1)+N(h-2),
\qquad N(-1)=0,\quad N(0)=1.
$$

이 점화식은 피보나치 수열보다 1만큼 이동한 형태여서 $$N(h)$$는 $$\varphi^h$$에 비례해 증가한다. 즉 어떤 양의 상수 $$c$$에 대해 $$N(h)\ge c\varphi^h$$이고,

$$
n\ge N(h)\ge c\varphi^h
\quad\Longrightarrow\quad
h\le \log_{\varphi}(n/c)=O(\log n).
$$

탐색·삽입·삭제는 루트에서 한 경로만 내려가므로 $$O(h)=O(\log n)$$이다. 이 보장은 모든 노드에서 $$\lvert\mathrm{BF}\rvert\le1$$을 유지할 때만 성립한다. 높이 갱신이나 회전을 빠뜨리면 트리가 편향되어 $$O(n)$$까지 나빠질 수 있다.

### 회전이 BST 순서를 보존하는 이유

LL 회전 전 key 구간을 $$A<y<B<z<C$$로 쓰면, 회전 전 inorder는 $$A,y,B,z,C$$다. `y`를 위로 올리고 `B`를 `z`의 왼쪽으로 옮긴 뒤에도 inorder는 $$A,y,B,z,C$$로 같다. 회전은 부모 관계만 바꾸며 key의 전체 순서를 바꾸지 않으므로 BST 불변식이 보존된다. LR/RL도 첫 회전으로 꺾인 구조를 직선 구조로 바꾼 뒤 같은 논리를 두 번 적용한다.
