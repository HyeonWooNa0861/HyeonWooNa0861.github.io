...
> **분류:** 이진 트리의 정의에서 따르는 **정리와 증명 개요**다. 루트 레벨을 0, 높이 $$h$$를 루트에서 가장 먼 리프까지의 간선 수로 정의한다.

> **원문 추적:** `LS10_binary_search_tree.pdf` pp.14–20은 레벨별 최대 노드 수, 높이별 합, 최소 레벨 수를 제시하고, pp.25–27은 완전 이진 트리의 배열 인덱스 식을 제시한다. 귀납법과 기하급수 전개는 작성자 보충이다.

레벨 0에는 노드가 최대 1개다. 각 노드는 자식을 최대 2개 가지므로, 레벨 $$i$$의 최대 노드 수가 $$2^i$$라면 다음 레벨은 최대 $$2\cdot2^i=2^{i+1}$$개다. 수학적 귀납법으로 레벨 $$i$$의 최대 노드 수는 $$2^i$$다. 이를 레벨 0부터 $$h$$까지 더하면

$$
N_{\max}(h)=\sum_{i=0}^{h}2^i
=\frac{2^{h+1}-1}{2-1}
=2^{h+1}-1.
$$

노드 $$n$$개를 담으려면 $$n\le2^{h+1}-1$$이어야 하므로

$$
h\ge\left\lceil\log_2(n+1)\right\rceil-1.
$$

따라서 최소 레벨 수 $$h+1$$은 $$\lceil\log_2(n+1)\rceil$$이다. 이 하한은 레벨을 빈틈없이 채울 때 달성되며, 편향 이진 트리는 높이가 $$n-1$$까지 커질 수 있다. 또 비어 있지 않은 트리에서 루트를 제외한 $$n-1$$개 노드는 정확히 하나의 부모 간선을 가지므로 간선 수도 $$n-1$$이다. 사이클이나 두 부모를 허용하면 더 이상 트리가 아니므로 이 결론을 적용할 수 없다.
...
이 식은 완전 이진 트리를 레벨 순서로 빈칸 없이 저장한다는 가정에서 나온다. 각 노드에 자식 자리 두 개를 연속 배정하면 노드 $$i$$의 두 자식은 $$2i+1,2i+2$$가 된다. 자식 인덱스 $$j>0$$에서 부모를 역산하면 홀수·짝수 모두 $$\lfloor(j-1)/2\rfloor$$로 합쳐진다. 중간 노드를 생략해 배열에 구멍을 허용하면 이 인덱스 관계는 그대로 쓸 수 없다.
...
같은 key 집합을 균형에 가까운 순서와 정렬된 순서로 삽입해 높이와 탐색 경로가 어떻게 달라지는지 비교한다. 재귀 find/insert에서는 null·key 일치 종료 조건과 return rt가 부모 링크를 복원하는 역할을 코드로 추적한다. BST 연산이 항상 O(log n)이 아니라 높이에 따라 최악 O(n)이 된다는 조건을 최우선으로 확인한다.
