...
> **작성자 보충 · 분류:** 이 식은 유한한 무향 tree에 대한 **정확한 등식**이다. $$\lvert V\rvert$$와 $$\lvert E_T\rvert$$는 단위가 없는 개수다.

> **원문 추적:** `LS23_minimum_spanning_tree.pdf` pp.8–9는 MST가 cycle 없이 $$\lvert V\rvert-1$$개 간선을 가짐을 제시한다. 아래 leaf-removal 귀납 증명은 작성자 보충이다.

Leaf removal로 귀납 증명할 수 있다. 정점이 하나인 tree는 간선이 0개이므로 $$0=1-1$$이다. 정점이 $$n>1$$개인 유한 tree에는 leaf가 존재한다. Leaf 하나와 그 leaf에 닿은 유일한 간선 하나를 제거하면 연결성과 acyclic 성질이 유지된 정점 $$n-1$$개의 tree가 남는다. 귀납 가정으로 남은 간선 수는 $$(n-1)-1=n-2$$이고, 제거한 간선을 다시 더하면

$$
\lvert E_T\rvert=(n-2)+1=n-1=\lvert V\rvert-1
$$

이다. 직관적으로 첫 정점 뒤의 각 새 정점은 cycle 없이 연결되려면 간선 하나씩을 정확히 요구한다. 그래프가 disconnected이면 spanning tree가 없고, cycle을 허용하거나 무한 그래프를 다루면 이 유한 tree 증명을 그대로 적용할 수 없다.
...
> **작성자 보충 · 단위:** 간선 weight $$w(u,v)$$, Prim의 key $$D[v]$$, tree의 total weight $$w(T)$$는 모두 같은 공통 단위 $$U$$를 사용해야 한다. $$U$$는 문제에 따라 원, km, ms처럼 정해지며, 정점·간선 수와 배열 index는 무차원이다. $$D[v]=\infty$$는 연결 간선을 아직 찾지 못했다는 sentinel이지 실제 weight가 아니며 total에 더하지 않는다. 서로 다른 단위의 weight를 변환 없이 비교하거나 합하면 MST의 최소성 의미가 깨진다.
...
> **분류:** MST의 cut property를 이용한 **교환 논증 증명 개요**다. 그래프가 연결된 무향 가중 그래프라고 가정한다.

> **원문 추적:** `LS23_minimum_spanning_tree.pdf` pp.16–27은 Prim의 선택·갱신 절차와 실행 예를 제시한다. 아래 cut-property 교환 논증은 greedy 선택의 안전성을 설명하는 작성자 보충이다.

현재 Prim이 선택한 정점 집합을 $$S$$, $$S$$와 $$V-S$$를 잇는 최소 가중 간선을 $$e$$라 하자. 현재까지 고른 간선을 포함하는 어떤 MST $$T$$가 $$e$$를 포함하지 않는다고 가정한다. $$T$$에 $$e$$를 추가하면 정확히 하나의 cycle이 생긴다. 이 cycle은 cut $$(S,V-S)$$를 다시 건너는 다른 간선 $$f$$를 반드시 포함한다.

$$e$$가 cut을 지나는 최소 간선이므로 $$w(e)\le w(f)$$다. 따라서

$$
T'=T-\{f\}+\{e\}
$$

는 여전히 모든 정점을 연결하고 cycle이 없는 spanning tree이며,

$$
w(T')=w(T)-w(f)+w(e)\le w(T).
$$

$$T$$가 최소였으므로 $$T'$$도 MST다. 즉 Prim이 고른 $$e$$를 포함하는 MST가 항상 존재해 greedy 선택은 안전하다. 가중치가 같으면 MST가 여러 개일 수 있지만 알고리즘은 그중 하나를 만든다. 그래프가 연결되지 않으면 하나의 spanning tree가 존재하지 않아 각 component의 minimum spanning forest만 얻는다. 방향 그래프에는 이 cut-property 기반 MST 정의를 그대로 적용할 수 없다.
...
연결 그래프에서 알고리즘이 끝나면 root를 제외한 각 정점의 최종 key는 그 정점을 tree에 붙인 선택 간선의 weight와 정확히 같다. 따라서

$$
w(T)=\sum_{v\in V-\{r\}}D[v]
$$

이다. 이 등식은 $$D[v]$$를 **정점이 선택될 때의 확정 key**로 읽어야 한다. 그래프가 disconnected여서 어떤 정점의 key가 $$\infty$$로 남거나, 선택 후 key를 다시 변경하는 잘못된 구현에서는 유한한 MST total을 뜻하지 않는다.
...
> **원문 추적:** `LS23_minimum_spanning_tree.pdf` p.28은 distance-table scan과 min-heap 방식의 비용을 Dijkstra와 같은 구조로 제시한다. 아래 연산별 합산은 그 결론을 풀어 쓴 작성자 보충이다.
...
<details markdown="block">
<summary markdown="span">2. 정점이 $$\lvert V\rvert$$개인 spanning tree의 간선 수는 몇 개인가?</summary>
...
<details markdown="block">
<summary markdown="span">5. Prim 알고리즘에서 $$S$$는 무엇을 의미하는가?</summary>
...
<details markdown="block">
<summary markdown="span">6. Prim 알고리즘에서 $$D[v]$$는 무엇을 의미하는가?</summary>
...
<details markdown="block">
<summary markdown="span">9. Prim에서 $$\text{prev}$$ 배열은 왜 필요한가?</summary>
