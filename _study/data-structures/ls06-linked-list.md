...
> **핵심:** **연결 리스트에서 중간 삽입이 $$O(n)$$인 이유는** 연결 자체는 $$O(1)$$이지만 이전 노드까지 탐색해야 한다. **`tail` 포인터의 장점은** 맨 뒤 삽입을 $$O(1)$$로 만든다.
...
빈 리스트, 하나뿐인 노드, 첫·마지막 노드 삭제를 각각 그림으로 그리고 head와 tail의 최종 값을 확인한다. 노드를 이미 알고 있을 때 연결 변경은 O(1)이지만 그 위치까지 탐색하면 O(n)이라는 비용 분리를 반드시 유지한다. 코드 복습은 새 노드의 next 설정 → 이전 연결 변경 → size 갱신 순으로 재현하고, 리스트가 비었을 때 tail 처리까지 검사한다.
...
<details markdown="block">
<summary markdown="span">2. 단일 연결 리스트에서 마지막 노드 삭제가 $$O(n)$$인 이유는 무엇인가?</summary>
...
### 연결 변경과 탐색 비용을 분리하는 증명

> **분류:** 포인터 연산 횟수를 세는 **복잡도 증명 개요**다. 각 노드의 `next` 접근과 포인터 대입을 상수 시간으로 가정한다.

> **원문 추적:** `LS06_linked_list.pdf` pp.25–36은 `addFirst`, `addLast`, 지정 위치 삽입·삭제·탐색의 코드와 $$O(1)$$·$$O(n)$$ 결론을 제시한다. 아래 $$T(i)$$ 식은 그 반복 횟수를 명시한 작성자 보충이다.

삽입 위치의 이전 노드 `prev`를 이미 알고 있다면 `new.next = prev.next`, `prev.next = new` 두 연결만 바꾸므로 $$\Theta(1)$$이다. 그러나 인덱스 $$i$$만 주어졌다면 `head`에서 `next`를 $$i-1$$번 따라가야 하므로

$$
T(i)=a(i-1)+b=\Theta(i),
$$

최악의 $$i=n$$에서는 $$\Theta(n)$$이다. 단일 연결 리스트의 마지막 노드를 삭제할 때도 마지막 직전 노드를 알아야 하므로 처음부터 $$n-1$$개를 따라가 $$\Theta(n)$$이다. `tail`은 마지막 **삽입**을 $$\Theta(1)$$로 만들지만 이전 노드를 알려 주지 않으므로 마지막 **삭제**까지 상수 시간으로 만들지는 못한다.

이 결론은 단일 연결 리스트와 인덱스만 주어진 인터페이스 기준이다. 이중 연결 리스트에서 삭제할 노드 참조를 직접 받으면 양쪽 링크를 상수 개만 바꿔 $$\Theta(1)$$에 삭제할 수 있다.
