...
> **핵심:** **병합 정렬의 기저 조건은** 리스트 길이 0 또는 1. **병합 연산이 $$O(n)$$인 이유는** 각 원소를 한 번씩 결과에 넣기 때문.
...
배열이 길이 1이 될 때까지 반으로 나누는 recursion tree를 그리고, 각 레벨에서 전체 n개 원소가 병합됨을 표시해 O(n log n)을 유도한다. merge 코드는 두 pointer 비교 후 한쪽 배열에 남은 원소를 붙이는 마지막 두 반복문까지 직접 재현한다. 항상 같은 시간 상한과 안정 정렬 가능성은 장점이지만 O(n) 보조 배열이 필요하다는 trade-off를 함께 기억한다.
...
> **분류:** 병합 정렬의 **점화식과 증명 개요**다. 설명을 단순화하려고 $$n=2^k$$, 비교·복사 한 번을 상수 시간으로 가정한다.

> **원문 추적:** `LS14_merge_sort.pdf` pp.52–55는 recursion depth $$\log n$$과 각 level의 총 $$n$$개 병합으로 $$O(n\log n)$$을 설명한다. 아래 점화식의 단계별 전개는 작성자 보충이다.

길이 $$n$$ 문제를 길이 $$n/2$$인 두 문제로 나누고, 두 결과를 병합하는 데 $$cn$$이 걸리므로
...
한 단계 전개하면 $$2[2T(n/4)+c(n/2)]+cn=4T(n/4)+2cn$$이다. $$j$$단계 뒤에는

$$
T(n)=2^jT(n/2^j)+jcn.
$$

$$j=\log_2n$$에서 부분 문제 크기가 1이므로
...
직관적으로도 각 레벨의 모든 부분 배열 길이 합이 $$n$$이고 레벨이 $$\log_2n$$개다. $$n$$이 2의 거듭제곱이 아니어도 두 부분의 크기는 floor/ceiling으로 한 칸만 달라져 같은 점근 결과다. 외부 정렬에서는 비교 횟수와 별개로 I/O 비용 모형을 분석해야 한다.
