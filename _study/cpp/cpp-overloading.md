...
| 원문 | 페이지 | 본문 대응 | 수식 판정 |
|---|---:|---|---|
| PDF | 1–9 | 함수·생성자·복사 대입, 깊은 복사, `static`, `const` | 7쪽의 동적 배열 크기를 정확한 byte 항등식으로 설명 |
| PDF | 10–18 | 연산자 종류, member/global overload, `friend` | 연산자 사용 규칙 중심 |
| PDF | 19–22 | `operator[]`, `operator()`, 2D 행렬의 1D 저장 | **핵심식**: 22쪽의 row/column-major offset을 일반화해 유도 |
| PDF | 23–24 | 파일 분리와 요약 | 새로운 수식 없음 |
| 추가 C++ 파일 | 전체 | 부록의 `Vector2D`·`Matrix2D` 코드와 실행 추적 | 행렬–벡터 성분 계산을 코드와 대응 |

PDF 24쪽 전체를 페이지 이미지로 대조하고 추가 C++ 파일도 원문과 대조했다. 반환형·시그니처 같은 언어 규칙은 수학식으로 바꾸지 않았고, 저장 위치와 행렬 곱처럼 실제 계산 의미가 있는 관계만 해설했다.
...
> **작성자 보충:** 아래 식은 연속된 `double` 원소 $$N$$개의 저장 크기를 바이트로 나타낸 정확한 할당 크기 항등식이다.

원문 7쪽의 `size * sizeof(double)` 형태에서 `size`를 원소 수 $$N$$이라 하면 필요한 저장 공간은

$$
B=N\times\operatorname{sizeof}(\texttt{double})\quad\text{bytes}
$$

다. $$N$$의 단위가 원소 개수이고 `sizeof(double)`의 단위가 바이트/원소이므로 결과는 바이트다. `malloc(size * sizeof(double))`은 이 바이트 수를 직접 받는 반면, `new double[size]`는 원소 수를 받아 같은 크기의 `double` 배열을 구성한다.

이 식은 $$N$$개의 `double`을 연속 저장한다는 조건에서 성립한다. C 방식으로 할당할 때는 `size <= SIZE_MAX / sizeof(double)`을 먼저 확인해 곱셈 overflow를 막고, 반환값이 null인지 확인해야 한다. C++의 `new double[size]`도 크기 계산이 표현 범위를 넘거나 메모리를 확보하지 못하면 정상 배열을 만들지 못한다. 기본 `new[]`는 메모리 확보 실패 시 `std::bad_alloc`을, 유효하지 않은 배열 길이에는 `std::bad_array_new_length`를 던질 수 있다. 확보한 배열은 할당 방식에 맞춰 각각 `free` 또는 `delete[]`로 해제해야 하며 둘을 섞으면 안 된다.
...
### 보충 해설: 일반 크기 행렬의 평탄화 공식

> **작성자 보충:** 원문 22쪽의 2x2 예제를 $$R$$행 $$C$$열의 연속 저장 행렬로 일반화한다. 원문 추가 코드의 `i + 2*j`는 아래 열 우선 공식에서 $$R=2$$인 특수한 경우다.

행 인덱스를 $$i$$, 열 인덱스를 $$j$$라고 하고 0부터 센다고 하자.

| 기호 | 정의 | 영역 | 단위 |
|---|---|---|---|
| $$R$$ | 전체 행 수 | 양의 정수 | 행 개수 |
| $$C$$ | 전체 열 수 | 양의 정수 | 열 개수 |
| $$i$$ | 접근할 행 | $$0 \le i < R$$ | 무차원 인덱스 |
| $$j$$ | 접근할 열 | $$0 \le j < C$$ | 무차원 인덱스 |
| $$k$$ | 1차원 배열 offset | $$0 \le k < RC$$ | 원소 개수 |

행 우선(row-major)에서는 행 $$i$$ 앞에 완성된 행이 $$i$$개 있고, 각 행마다 원소가 $$C$$개 있다. 그 뒤 현재 행에서 $$j$$개만큼 이동하므로

$$
k_{\text{row}} = \underbrace{iC}_{\text{앞선 }i\text{개 행}} + \underbrace{j}_{\text{현재 행 안의 이동}}
$$

이다. 열 우선(column-major)에서는 열 $$j$$ 앞에 완성된 열이 $$j$$개 있고, 각 열마다 원소가 $$R$$개 있다. 현재 열에서 행 $$i$$만큼 이동하므로

$$
k_{\text{column}} = \underbrace{jR}_{\text{앞선 }j\text{개 열}} + \underbrace{i}_{\text{현재 열 안의 이동}}
= i+jR
$$

이다. 원문의 2x2 코드에서는 $$R=C=2$$이므로 열 우선 offset이 $$i+2j$$가 된다. `data`의 시작 주소를 $$p$$, 원소 타입을 `T`라고 하면 실제 바이트 주소는

$$
\operatorname{address}(i,j)=p+k\,\operatorname{sizeof}(T)
$$

이며, 여기서 $$k$$는 원소 단위 offset이고 주소 식의 결과는 바이트 단위다.

두 offset 식은 **0 기반 인덱스, 고정된 행·열 크기, 원소 사이에 별도 padding이 없는 연속 1차원 저장**에서 정확한 등식이다. 인덱스가 범위를 벗어나거나, 행 우선 데이터에 열 우선 식을 쓰거나, 행마다 길이가 다르거나, 실제 stride가 $$R$$ 또는 $$C$$와 다른 padded 배열이면 사용할 수 없다. 이때는 실제 leading dimension을 $$L$$이라 두고 행 우선은 $$iL+j$$, 열 우선은 $$i+jL$$처럼 저장소의 stride를 써야 한다. 또한 큰 크기에서는 $$RC$$와 offset 계산이 인덱스 타입의 범위를 넘지 않는지도 확인해야 한다.
...
추가 C++ 파일의 이 계산은 행렬–열벡터 곱의 정의를 그대로 전개한 것이다. $$A\in\mathbb{R}^{2\times2}$$, $$x\in\mathbb{R}^{2}$$라 하면 결과 $$y=Ax$$의 $$i$$번째 성분은

$$
y_i=(Ax)_i=\sum_{j=0}^{1}A_{ij}x_j
$$

이다. 열이 두 개뿐이므로 합을 직접 펼치면

$$
y_0=A_{00}x_0+A_{01}x_1,\qquad
y_1=A_{10}x_0+A_{11}x_1
$$

가 되고, 이것이 두 `ret` 대입문의 오른쪽과 항별로 일치한다. 이는 2x2 행렬과 길이 2 열벡터에 대한 **정의에서 얻는 정확한 등식**이다. 인덱스가 0과 1의 범위를 벗어나지 않고, `A(i,j)`가 위에서 설명한 같은 행·열 원소를 반환한다는 조건이 필요하다.
...
이번에는 `v`를 행 벡터로 해석한다. 결과 $$z=x^{\top}A$$의 $$j$$번째 성분 정의는

$$
z_j=(x^{\top}A)_j=\sum_{i=0}^{1}x_iA_{ij}
$$

이고, 두 행의 항을 펼치면

$$
z_0=x_0A_{00}+x_1A_{10},\qquad
z_1=x_0A_{01}+x_1A_{11}
$$

가 된다. 따라서 첫 번째 `ret` 문은 행렬의 0번 열과, 두 번째 `ret` 문은 1번 열과의 성분곱 합을 계산한다. 피연산자 순서가 바뀌면 합산하는 인덱스도 바뀌므로 일반적으로 $$Ax$$와 $$x^{\top}A$$는 같은 연산이 아니다.
