...
|---|---|---|
| pp. 1–8 | autoregressive recap, latent variables, Gaussian example | p. 8의 Gaussian parameterization은 모델 정의; 별도 증명 대상 없음 |
| pp. 9–14 | graphical model, mixture of Gaussians, VAE family | p. 12의 mixture와 p. 13의 VAE density를 본문에 명시 |
...
### 핵심 수식 유도: finite-$$K$$ importance sampling과 log bias

> **Source mapping:** Official Lecture 5 PDF p. 19의 finite-sample importance estimator와 unbiasedness, p. 20의 log-likelihood estimator와 expectation/log 불일치에 대응한다. Finite-variance 조건과 delta-method 오차식은 작성자 보충이다.

Weight를 $$w(z)=p_\theta(x,z)/q(z)$$로 두고 $$z^{(1)},\ldots,z^{(K)}\overset{\mathrm{iid}}\sim q$$를 뽑으면 evidence의 finite-sample estimator는

$$
\widehat p_K(x)
=\frac1K\sum_{k=1}^{K}w\!\left(z^{(k)}\right)
=\frac1K\sum_{k=1}^{K}\frac{p_\theta(x,z^{(k)})}{q(z^{(k)})}
$$

이다. $$p_\theta(x,z)>0$$인 곳에서 $$q(z)>0$$이고 $$\mathbb E_q[\lvert w\rvert]<\infty$$이면

$$
\mathbb E_q[\widehat p_K(x)]
=\frac1K\sum_{k=1}^{K}\mathbb E_q[w(z^{(k)})]
=\int p_\theta(x,z)\,dz
=p_\theta(x).
$$

따라서 $$\widehat p_K$$는 $$p_\theta(x)$$의 **unbiased Monte Carlo estimator**다. 이 결론은 i.i.d. sampling과 support coverage에 의존한다. 더 나아가 $$\operatorname{Var}_q(w)<\infty$$이면

$$
\operatorname{Var}(\widehat p_K)=\frac{\operatorname{Var}_q(w)}{K},
$$

즉 standard error는 $$K^{-1/2}$$ 비율로 감소한다. $$q(z)=p_\theta(z\mid x)$$이면 $$w(z)=p_\theta(x)$$가 상수여서 variance가 0이다. 반대로 $$q$$가 중요한 posterior 영역을 거의 방문하지 않거나 weight의 second moment가 무한하면 estimator는 형식상 unbiased여도 finite-$$K$$에서 매우 불안정하다.

Training에 필요한 natural-log likelihood를 plug-in으로 추정하면

$$
\widehat\ell_K(x)=\log\widehat p_K(x)
$$

이다. 이것은 sample을 고정한 계산값으로는 정확한 log 변환이지만, $$\widehat p_K>0$$ almost surely이고 log expectation이 존재할 때 $$\log$$가 concave이므로 **log-likelihood에 대한 unbiased estimator가 아니다**.

$$
\mathbb E[\widehat\ell_K(x)]
\le \log\mathbb E[\widehat p_K(x)]
=\log p_\theta(x).
$$

Weight가 양수이고 variance가 작을 때 second-order delta method는 bias와 variance를

$$
\mathbb E[\widehat\ell_K]-\log p_\theta(x)
\approx-\frac{\operatorname{Var}_q(w)}{2Kp_\theta(x)^2},
\qquad
\operatorname{Var}(\widehat\ell_K)
\approx\frac{\operatorname{Var}_q(w)}{Kp_\theta(x)^2}
$$

로 준다. 두 식은 finite-$$K$$ **근사**이며 exact identity가 아니다. 적절한 적분가능성과 $$p_\theta(x)>0$$ 아래에서 $$K\to\infty$$이면 law of large numbers와 continuity로 $$\widehat\ell_K\to\log p_\theta(x)$$가 되지만, finite $$K$$에서는 Jensen gap이 남는다. 모든 sampled weight가 0이면 $$\log\widehat p_K=-\infty$$이고, heavy-tailed weight에서는 위 delta approximation도 실패할 수 있다. $$K$$와 sample index는 무차원이다. Discrete probability mass에서는 weight와 evidence가 무차원이다. Continuous $$z$$에서 numerator와 proposal density는 같은 latent reference measure를 사용해야 하며, $$w=p(x,z)/q(z)$$는 $$x$$에 대한 marginal density의 단위를 남긴다. 따라서 continuous log-density의 수치는 고정된 $$x$$ reference measure와 unit convention 아래에서만 비교한다.
...
&=\log\mathbb{E}_{q(z\mid x)}\left[\frac{p_\theta(x,z)}{q(z\mid x)}\right]\\
&\ge \mathbb{E}_{q(z\mid x)}\left[\log p_\theta(x,z)-\log q(z\mid x)\right]
=\mathcal{L}(x).
\end{aligned}
$$

마지막 부등식은 concave $$\log$$에 대한 Jensen 부등식이다. 더 정확히는

$$
\log p_\theta(x)-\mathcal{L}(x)
=D_{\mathrm{KL}}\!\left(q(z\mid x)\Vert p_\theta(z\mid x)\right)\ge 0.
$$

따라서 $$q$$가 true posterior와 같을 때만 bound가 tight하다. $$x$$는 관측값, $$z$$는 latent variable, $$q$$는 variational posterior, $$\theta$$는 generative parameter이며 모두 물리 단위가 없는 확률모형 기호다. Variational family가 posterior를 표현하지 못하면 inference gap이 남고, 높은 ELBO가 곧 좋은 perceptual sample을 보장하지도 않는다.
