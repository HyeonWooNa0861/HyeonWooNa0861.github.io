...
<div class="branch-catalog">
  {% for group in research_groups %}
    <section class="branch-catalog__group" aria-labelledby="research-group-{{ forloop.index }}">
      <header class="branch-catalog__heading">
        <h2 id="research-group-{{ forloop.index }}">{{ group.name }}</h2>
        <span>{{ group.items | size }} notes</span>
      </header>
      <ul class="post-list content-catalog-list">
        ...
        {% for note in research_items %}
          <li class="post-card content-card">
            <a class="branch-card-link" data-label="{{ note.title | escape }}" href="{{ note.url | relative_url }}">{{ note.title }}</a>
            <p>{{ note.topic }}</p>
